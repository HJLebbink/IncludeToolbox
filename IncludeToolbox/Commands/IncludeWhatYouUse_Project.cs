...
namespace IncludeToolbox.Commands
{
    ...
    /// Command handler
    /// </summary>
    internal sealed class IncludeWhatYouUse_Project : CommandBase<IncludeWhatYouUse_Project>
    {
        public override CommandID CommandID => new CommandID(CommandSetGuids.ProjectGroup, 0x0100);

        private IncludeWhatYouUse impl;
        ...
        public IncludeWhatYouUse_Project()
        {
        ...
        private async Task<bool> ProcessNextFile()
        {
            ...
            while (projectFiles.Count > 0)
            {
                ...
                bool started = await impl.Perfor TrialAndErrorIncludeRemoval(document, settings);
                if (started)
                    return true;
            }
            return false;
